...
class ArgumentGroupFactory:
    ...
    def __init__(
        self, src_cfg_class: type, exclude: Optional[list[str]] = None
    ) -> None:
    ...
    def _format_arg_name(
        self, config_attr_name: str, prefix: Optional[str] = None
    ) -> str:
    ...
    def _extract_type(self, config_type: type) -> dict[str, Any]:
        ...
        type_tuple = get_args(config_type)
        ...
        if origin in [types.UnionType, Union]:
            ...
            if type_tuple[1] == type(
                ...
                raise TypeInferenceError(
                    f"Unions not supported by argparse: {config_type}"
                )
    ...
    def _build_argparse_kwargs_from_field(self, attribute: Field) -> dict[str, Any]:
        ...
        argparse_kwargs["help"] = (
            self.field_docstrings[attribute.name]
            if attribute.name in self.field_docstrings
            else ""
        )
    ...
    def build_group(
        self, parser: ArgumentParser, title: Optional[str] = None
    ) -> _ArgumentGroup:
        ...
        arg_group = parser.add_argument_group(
            title=title, description=self.src_cfg_class.__doc__
        )
...
def add_config(parser, config, title):
    transformer_factory = ArgumentGroupFactory(config)
    transformer_factory.build_group(parser, title)
    return parser
...
def parse_args(
    ...
    parser = parser or argparse.ArgumentParser(
        description=f"{title} Arguments", allow_abbrev=False
    )

    parser = add_config(parser, config, title)
